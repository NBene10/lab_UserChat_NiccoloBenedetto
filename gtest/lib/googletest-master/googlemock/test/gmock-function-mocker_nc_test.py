...
class GMockMethodNCTest(googletest.TestCase):
    ...
    # The class body is intentionally empty.  The actual gtest*() methods
    # will be defined at run time by a call to
    # DefineNegativeCompilationTests() later.
    pass


# Defines a list of gtest specs, where each element is a tuple
# (gtest name, list of regexes for matching the compiler errors).
TEST_SPECS = [
    ("MOCK_METHOD_INVALID_CONST_SPEC",
     [r"onst cannot be recognized as a valid specification modifier"]),
]

# Define a gtest method in GMockNCTest for each element in TEST_SPECS.
fake_target_util.DefineNegativeCompilationTests(
    GMockMethodNCTest,
    "google3/third_party/googletest/googlemock/gtest/gmock-function-mocker_nc",
