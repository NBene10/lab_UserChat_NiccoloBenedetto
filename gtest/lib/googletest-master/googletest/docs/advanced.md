...
TEST(MyDeathTest, Foo) {
  // This death gtest uses a compound statement.
  ASSERT_DEATH({
    int n = 5;
    Foo(&n);
...
TEST(MyDeathTest, TestOne) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
  // This gtest is run in the "threadsafe" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}

TEST(MyDeathTest, TestTwo) {
  // This gtest is run in the "fast" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}
```
...
TEST(FooTest, Bar) {
  Subroutine();  // The intended behavior is for the fatal failure
                 // in Subroutine() to abort the entire gtest.
...
class FooTest : public ::testing::Test {
 protected:
  // Per-gtest-suite set-up.
  // Called before the first gtest in this gtest suite.
  // Can be omitted if not needed.
  static void SetUpTestSuite() {
    shared_resource_ = new ...;
  }

  // Per-gtest-suite tear-down.
  // Called after the last gtest in this gtest suite.
  // Can be omitted if not needed.
  ...
  // You can define per-gtest set-up logic as usual.
  virtual void SetUp() { ... }

  // You can define per-gtest tear-down logic as usual.
...
class FooTest :
  ...
  // To access the gtest parameter, call GetParam() from class
  // TestWithParam<T>.
};
...
class TestInfo {
 public:
  // Returns the gtest suite name and the gtest name, respectively.
  //
...
```c++
  // Gets information about the currently running gtest.
  ...
  printf("We are in gtest %s of gtest suite %s.\n",
         test_info->name(),
